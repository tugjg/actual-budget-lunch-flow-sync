...
class LunchFlowImporter {
    ...
    updateClients() {
        if (this.config) {
            ...
            this.abClient = new actual_budget_client_1.ActualBudgetClient(this.config.actualBudget.serverUrl, this.config.actualBudget.budgetSyncId, this.config.actualBudget.password, this.config.actualBudget.encryptionPassword);
        }
    }
    ...
    async importTransactions(skipConfirmation = false, throwOnError = false) {
        ...
        try {
            ...
            const allLfTransactions = [];
            const accountResults = [];
            for (const mapping of this.config.accountMappings) {
                try {
                    const accountTransactions = await this.lfClient.getTransactions(mapping.lunchFlowAccountId, mapping.includePending ?? false);
                    // Filter transactions by sync start date if specified
                    let filteredTransactions = accountTransactions;
                    if (mapping.syncStartDate) {
                        filteredTransactions = accountTransactions.filter(transaction => transaction.date >= mapping.syncStartDate);
                    }
            ...
            if (this.config.actualBudget.duplicateCheckingAcrossAccounts) {
                ...
                try {
                    ...
                    const duplicateDetector = new duplicate_detector_1.DuplicateTransactionDetector(existingTransactions);
